...
type ResponseType = InferResponseType<typeof client.api.appointments.$post>
//what the endpoint is expecting to be sent which needs the zValidator from the api accessed by json
type RequestType = InferRequestType<typeof client.api.appointments.$post>["json"]
...
export const useCreateFacility = () => {
    ...
    > ({
        ...
        mutationFn: async (json) => {
            const response = await client.api.appointments.$post({ json })
